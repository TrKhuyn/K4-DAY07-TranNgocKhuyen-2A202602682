...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split at whitespace *after* sentence-ending punctuation, preserving
        # that punctuation as part of the preceding sentence.
        sentences = [
            sentence.strip()
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # With no usable separators left, character-level splitting is the
        # safe fallback that guarantees progress.
        ...
        for index, piece in enumerate(pieces):
            # Restore the separator to every piece except the final one, so
            # sentence/paragraph boundaries are retained in the output.
            candidate = piece + (separator if index < len(pieces) - 1 else "")
            if len(candidate) > self.chunk_size:
                if buffer.strip():
                    chunks.append(buffer.strip())
                    buffer = ""
                chunks.extend(self._split(candidate, later_separators))
